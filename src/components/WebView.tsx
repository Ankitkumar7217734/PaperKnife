...
const ToolCard = ({ title, desc, icon: Icon, onClick, category, isNew }: Tool & { onClick?: () => void }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="group relative flex flex-col p-4 md:p-6 rounded-2xl md:rounded-[2rem] bg-pk-surface dark:bg-zinc-900/40 border border-pk-border dark:border-white/5 hover:border-rose-300 dark:hover:border-rose-500/50 transition-all duration-300 text-left shadow-pk-sm hover:shadow-pk-md dark:shadow-none hover:-translate-y-0.5"
    >
      ...
      <h3 className="font-black text-stone-900 dark:text-white mb-1.5 md:mb-2 text-sm md:text-lg tracking-tight group-hover:text-rose-500 transition-colors">{title}</h3>
      <p className="text-xs md:text-sm text-stone-500 dark:text-zinc-400 font-medium leading-relaxed line-clamp-2">{desc}</p>
...
const pillActive: Record<'All' | ToolCategory, string> = {
  All: 'bg-stone-900 dark:bg-white text-white dark:text-black shadow-pk-sm',
  Edit: 'bg-rose-500 text-white shadow-pk-sm',
  Secure: 'bg-indigo-500 text-white shadow-pk-sm',
  Convert: 'bg-emerald-500 text-white shadow-pk-sm',
  Optimize: 'bg-amber-500 text-white shadow-pk-sm'
}
...
export default function WebView({ tools }: { tools: Tool[] }) {
  ...
  return (
    <div className="min-h-screen bg-pk-canvas">
      {/* Hero Section */}
      <section className="relative pt-10 md:pt-14 pb-8 md:pb-10 px-6 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,var(--pk-glow),transparent_72%)] pointer-events-none" />

        <div className="max-w-6xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-5 border border-rose-200/80 dark:border-rose-900/30">
            <SparklesIcon size={14} /> Professional PDF Engine
          </div>
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-stone-900 dark:text-white mb-4 leading-[0.9]">
            Stop Uploading <br/>
          ...
          <div className="max-w-2xl mx-auto relative group mt-6 md:mt-8">
            ...
            <input
              ...
              className="w-full bg-pk-surface dark:bg-zinc-900 border border-pk-border dark:border-white/5 rounded-[2rem] py-4 md:py-5 pl-14 pr-6 md:pr-16 shadow-pk-md dark:shadow-none focus:border-rose-400 focus:ring-4 focus:ring-rose-500/10 outline-none transition-all font-bold text-base md:text-lg text-stone-900 dark:text-white"
            />
            <kbd className="hidden md:flex absolute right-5 inset-y-0 my-auto h-7 items-center px-2.5 rounded-lg border border-pk-border dark:border-zinc-700 text-[10px] font-black text-stone-300 dark:text-zinc-600 pointer-events-none">/</kbd>
          </div>
          <p className="hidden md:flex items-center justify-center gap-2 mt-4 text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 dark:text-zinc-600">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {recentTools.map(tool => (
                    <button
                      ...
                      className="flex items-center gap-2.5 pl-2.5 pr-4 py-2 bg-pk-surface dark:bg-zinc-900 border border-pk-border dark:border-white/5 rounded-2xl shadow-pk-sm hover:border-rose-300 dark:hover:border-rose-500/50 hover:-translate-y-0.5 transition-all group"
                    >
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 md:px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all border ${activeCategory === cat ? `${pillActive[cat]} border-transparent` : 'bg-pk-surface dark:bg-zinc-900 text-stone-400 border-pk-border dark:border-white/5 hover:border-rose-300 dark:hover:border-rose-500'}`}
                  >
