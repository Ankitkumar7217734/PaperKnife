  ...
  return (
    <div className="min-h-screen pk-shell pb-32">
      <header className="px-6 pt-[calc(env(safe-area-inset-top)+0.75rem)] pb-6">
        <h1 className="text-4xl font-black tracking-tighter text-stone-900 dark:text-white mb-8">All Tools</h1>
      ...
      <main className="px-4 space-y-8">
        {(Object.keys(groupedTools) as ToolCategory[]).map((category) => (
          <section key={category} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h3 className="px-2 mb-4 text-[11px] font-black uppercase tracking-[0.2em] text-stone-500 dark:text-zinc-400">
              {category} Tools
            </h3>
              ...
              {groupedTools[category].map((tool, i) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-4 p-4 pk-surface-card rounded-2xl active:bg-pk-surface-muted dark:active:bg-zinc-950 transition-all"
                  >
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <p className="text-xs text-stone-500 dark:text-zinc-400 truncate">{tool.desc}</p>
                    </div>
      ...
      <footer className="text-center py-12 opacity-20">
         ...
         <p className="text-[9px] font-black uppercase tracking-[0.5em]">PaperKnife Version 1.3.0</p>
      </footer>
    </div>
  )
}
