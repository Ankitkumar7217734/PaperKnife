...
export default function SplitTool() {
  ...
  const handleUnlock = async () => {
    ...
    if (result.success) {
      ...
      setStartPage('1'); setEndPage(String(result.pageCount))
    } else {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (meta.isLocked) {
        ...
        setStartPage('1'); setEndPage(String(meta.pageCount))
      }
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const applyStartEnd = () => {
    ...
    if (isNaN(start) || isNaN(end)) {
      toast.error('Enter start and end page numbers')
      return
    }
    if (start < 1 || end > pdfData.pageCount || start > end) {
      toast.error(`Range must be between 1 and ${pdfData.pageCount}`)
      return
    }
                ...
                <div className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-[8px] text-gray-400 mt-2 px-1">Selects pages from Start through End (inclusive).</p>
                    </div>
                    <div>
