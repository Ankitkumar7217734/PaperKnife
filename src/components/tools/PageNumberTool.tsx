      ...
      {!pdfData ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-pk-surface dark:bg-zinc-900 p-6 rounded-[2rem] border border-pk-border dark:border-white/5 shadow-sm flex flex-col items-center">
              ...
              {!downloadUrl ? (
                <>
                  <div>
                    ...
                    <input type="text" value={format} onChange={(e) => setFormat(e.target.value)} className="w-full bg-pk-surface-muted dark:bg-zinc-950 rounded-xl px-4 py-3 border border-transparent focus:border-rose-500 outline-none font-bold text-sm dark:text-white" placeholder="Page {n} of {total}" />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-center', 'bottom-right'] as Position[]).map(pos => (
                        <button key={pos} onClick={() => setPosition(pos)} className={`py-3 px-1 rounded-xl text-[8px] font-black uppercase transition-all border ${position === pos ? 'bg-rose-500 text-white border-rose-500 shadow-lg shadow-rose-500/20' : 'bg-pk-surface-muted dark:bg-zinc-950 text-gray-400 border-pk-border dark:border-zinc-800'}`}>
                          {pos.replace('-', ' ')}
                        </button>
                      ))}
                    </div>
