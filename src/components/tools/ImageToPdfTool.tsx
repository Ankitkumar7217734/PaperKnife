...
function SortableImageItem({ id, img, onRemove }: { id: string, img: ImageFile, onRemove: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 p-3 bg-pk-surface dark:bg-zinc-900 rounded-2xl border transition-colors shadow-sm group touch-none relative ${isDragging ? 'border-rose-500 shadow-xl scale-[1.02]' : 'border-pk-border dark:border-white/5'}`}>
...
export default function ImageToPdfTool() {
  ...
  return (
    <NativeToolLayout title="Image to PDF" description="Convert photos and images into a professional PDF." actions={images.length > 0 && !downloadUrl && <ActionButton />}>
      ...
      {images.length === 0 ? (
        <div onClick={() => fileInputRef.current?.click()} className="border-4 border-dashed border-pk-border dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-rose-50 dark:hover:bg-rose-900/10 transition-all cursor-pointer group">
